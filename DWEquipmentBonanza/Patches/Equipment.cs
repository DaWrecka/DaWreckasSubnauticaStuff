...
namespace DWEquipmentBonanza.Patches
{
	...
	internal class EquipmentPatches
	{
		...
		public readonly static Dictionary<TechType, HashSet<TechType> > Substitutions = new Dictionary<TechType, HashSet<TechType> >();
		...
		public static void AddSubstitution(TechType custom, TechType vanilla)
		{
			...
			{
				set.Add(custom);
				//Log.LogDebug($"EquipmentPatches.AddSubstitution: Added sub {custom.AsString()} to existing HashSet for TechType {vanilla.AsString()}, new count for this vanilla TechType {set.Count}");
				return;
			}
			...
			//Log.LogDebug($"EquipmentPatches.AddSubstitution: Added new substitution set for TechType {vanilla.AsString()} with sub {custom.AsString()}");
			//substitutionTargets.Add(substitution);

		}

		public static void AddSubstitutions(TechType custom, ICollection<TechType> substitutions)
		{
		...
		{
			//Log.LogDebug($"EquipmentPatches.PostGetCount: executing with parameters __result {__result.ToString()}, techType {techType.AsString()}");
			if (!Substitutions.ContainsKey(techType))
			{
				//Log.LogDebug($"EquipmentPatches.PostGetCount: TechType not found in Substitutions");
			...
			{
				//Log.LogDebug($"EquipmentPatches.PostGetCount: got substitution set with {subs.Count} members");
				foreach (TechType sub in subs)
				{
					if (equipCount.TryGetValue(sub, out int c))
					{
						//Log.LogDebug($"EquipmentPatches.PostGetCount: found TechType {sub.AsString()} equipped {c} times");
						__result += c;
					}
					else
					{
						//Log.LogDebug($"EquipmentPatches.PostGetCount: TechType {sub.AsString()} not found in equipment.");
					}
					//__result += equipCount.GetOrDefault(sub, 0);
				}
			}
			//Log.LogDebug($"EquipmentPatches.PostGetCount: finished with result {__result.ToString()}");
		}
