...
namespace DWEquipmentBonanza.Patches
{
	...
	{
		protected static readonly Dictionary<string, float> cooldownMultipliers = new Dictionary<string, float>();

		public static bool AddOrUpdateCooldownMultiplier(Vehicle v, float multiplier)
		{
			PrefabIdentifier pi = v.GetComponentInParent<PrefabIdentifier>();
			if (pi != null)
			{
				string id = pi.Id;
				if (!string.IsNullOrEmpty(id))
				{
					cooldownMultipliers[id] = multiplier;
					return true; 
				}
			}
