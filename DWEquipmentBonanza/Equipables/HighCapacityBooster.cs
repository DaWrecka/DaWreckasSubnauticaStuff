...
namespace DWEquipmentBonanza.Equipables
{
	...
	internal class HighCapacityBooster : Equipable
	{
		...
		protected override TechType templateType => TechType.SuitBoosterTank;
#endif
		protected static float OxygenCapacity = -1f;
		...
		public virtual IEnumerator ModPrefabAsync(GameObject gameObject)
		{
			if (OxygenCapacity == -1f)
			{
				CoroutineTask<GameObject> task = CraftData.GetPrefabForTechTypeAsync(TechType.HighCapacityTank);
				yield return task;
				var tankPrefab = task.GetResult();

				OxygenCapacity = tankPrefab.GetComponent<Oxygen>().oxygenCapacity;
			}

			gameObject.GetComponent<Oxygen>().oxygenCapacity = OxygenCapacity;
		}
...
#if NAUTILUS
		public override IEnumerator ModPrefabAsync(GameObject gameObject)
		{
			yield return base.ModPrefabAsync(gameObject);
