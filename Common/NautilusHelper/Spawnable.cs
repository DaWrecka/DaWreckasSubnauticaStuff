	...
	public abstract class Spawnable
	{
		...
		public void Patch()
		{
			...
			else
			{
				...
				throw new ArgumentException($"Clone template not specified for Spawnable {this.ClassID}");
			}
		...
		protected virtual Sprite GetItemSprite()
		{
			...
			if (HasSprite)
				Log.LogError($"Sprite for '{this.prefabFileName}': Did not find an image file at '{path}'");
