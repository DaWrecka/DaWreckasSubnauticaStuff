...
using System.Text;
using UnityEngine;
using PDAEncyclopediaHandler = Nautilus.Handlers.PDAHandler;
...
namespace Common.NautilusHelper
{
	public abstract class PdaItem : Spawnable
	{
		...
		internal string DiscoverMessageResolved => DiscoverMessage == null ? "NotificationBlueprintUnlocked" : $"{this.TechType.AsString()}_DiscoverMessage";
		...
		public override void FinalisePrefab(CustomPrefab prefab)
		{
			...
			if (!prefab.TryGetGadget(out scanningGadget))
			{
				scanningGadget = prefab.AddGadget(new ScanningGadget(prefab, RequiredForUnlock, FragmentsToScan));
			}

			if(CompoundTechsForUnlock != null)
			{
				scanningGadget.WithCompoundTechsForUnlock(CompoundTechsForUnlock);
			}

			scanningGadget.WithPdaGroupCategory(GroupForPDA, CategoryForPDA);
			if(EncyclopediaEntryData != null)
			{
				scanningGadget.WithEncyclopediaEntry(EncyclopediaEntryData.path, EncyclopediaEntryData.popup, EncyclopediaEntryData.image, EncyclopediaEntryData.sound, EncyclopediaEntryData.audio);
			}

			this.prefabInfo.WithSizeInInventory(SizeInInventory);
		}
		...
		{
			/*CraftDataHandler.SetRecipeData(this.TechType, GetBlueprintRecipe());
			...
			if (GroupForPDA != TechGroup.Uncategorized)
			{
				...
				{
					Log.LogError($"Failed to add {this.TechType.AsString()} to {GroupForPDA}/{CategoryForPDA} as it is not a registered combination.");
				}
			}

			if (EncyclopediaEntryData != null)
			{
				PDAEncyclopediaHandler.AddEncyclopediaEntry(EncyclopediaEntryData);
			}
			...
			if (!UnlockedAtStart)
			{
				...
				KnownTechHandler.SetAnalysisTechEntry(unlockTech, new TechType[1] { this.TechType }, DiscoverMessageResolved);
		...
		/*internal sealed override void PatchTechType()
		{
			...
			this.TechType = EnumHandler.AddEntry<TechType>(ClassID)
				...
				.WithPdaInfo(FriendlyName, Description, "English", unlockAtStart: UnlockedAtStart)
