...
#region[Declarations]
		public const string
			...
			public const string version = "0.25.0.5";
#endregion
		...
		private const float bioFuelCellMultiplier = 1.2f;
		private const float ionFuelCellMultiplier = 0.8f;
		private const float IonFuelCellRechargeInterval = 0.5f;
		private const float IonFuelCellSmallRechargeRate = 0.5f;
		private const float IonFuelCellLargeRechargeRate = 1f;
		private int fuelCellCapacity(bool large = false) => (int)(config.smallFuelCellCap * (large ? 2 + config.cellMultiplier : 1));
		private int bioCellCapacity(bool large = false) => (int)(config.smallFuelCellCap * (large ? 2 + config.cellMultiplier : 1));
		private int ionCellCapacity(bool large = false) => (int)(config.smallFuelCellCap * ionFuelCellMultiplier * (large ? 2 : 1));
		...
		private bool VerifyBattery(TechType battery) => BatteryCharger.compatibleTech.Add(battery);
		private bool VerifyPowerCell(TechType cell) => PowerCellCharger.compatibleTech.Add(cell);
		...
		public void Start()
		{
			...
			if (plasmaCoreType != TechType.None)
			{
				...
				{
					EnergyCapacity = bioCellCapacity(),
					ID = "BioFuelBattery",
				...
				{
					EnergyCapacity = bioCellCapacity(true),
					ID = "BioFuelCell",
				...
				VerifyBattery(bioFuelBattery.TechType);
				VerifyPowerCell(bioFuelCell.TechType);
				RegeneratingPowerSource.StaticSetRegenParameters(0.5f, 1f, bioFuelBattery.TechType);
				RegeneratingPowerSource.StaticSetRegenParameters(0.5f, 2f, bioFuelCell.TechType);
			}

			var ionFuelCellSmall = new CbBattery
			{
				EnergyCapacity = ionCellCapacity(),
				ID = "IonFuelCellSmall",
				Name = "Small Ion Fuel Cell",
				FlavorText = "Precursor ion technology combined with a modified fuel cell creates a self-recharging portable power source",
				CraftingMaterials = { nBattery.TechType, TechType.PrecursorIonBattery },
				UnlocksWith = TechType.PrecursorIonBattery,
				CustomIcon = ImageUtils.LoadSpriteFromFile(Path.Combine(AssetsFolder, "precursorionfuelcellsmall.png")),
				CBModelData = new CBModelData
				{
					CustomTexture = ImageUtils.LoadTextureFromFile(Path.Combine(AssetsFolder, "battery_ionfuelcell.png")),
					CustomIllumMap = ImageUtils.LoadTextureFromFile(Path.Combine(AssetsFolder, "battery_ionfuelcell_illum.png")),
					CustomSpecMap = ImageUtils.LoadTextureFromFile(Path.Combine(AssetsFolder, "battery_ionfuelcell_spec.png")),
					UseIonModelsAsBase = true,
					//CustomIllumStrength = 1.2f
				},
				EnhanceGameObject = new Action<GameObject>((go) =>
				{
					var regenerator = go.EnsureComponent<RegeneratingPowerSource>();
					regenerator.SetRegenParameters(IonFuelCellRechargeInterval, IonFuelCellSmallRechargeRate);
				})
			};
			ionFuelCellSmall.Patch();
			RegeneratingPowerSource.StaticSetRegenParameters(IonFuelCellRechargeInterval, IonFuelCellSmallRechargeRate, ionFuelCellSmall.TechType);

			var ionFuelCellLarge = new CbPowerCell
			{
				EnergyCapacity = ionCellCapacity(true),
				ID = "IonFuelCellLarge",
				Name = "Large Ion Fuel Cell",
				FlavorText = "Precursor ion technology combined with a modified fuel cell creates a self-recharging portable power source",
				CraftingMaterials = { nPowercell.TechType, TechType.PrecursorIonPowerCell },
				UnlocksWith = TechType.PrecursorIonBattery,
				CustomIcon = ImageUtils.LoadSpriteFromFile(Path.Combine(AssetsFolder, "precursorionfuelcell.png")),
				CBModelData = new CBModelData
				{
					CustomTexture = ImageUtils.LoadTextureFromFile(Path.Combine(AssetsFolder, "ion_fuel_cell_01.png")),
					CustomIllumMap = ImageUtils.LoadTextureFromFile(Path.Combine(AssetsFolder, "ion_fuel_cell_01_illum.png")),
					CustomSpecMap = ImageUtils.LoadTextureFromFile(Path.Combine(AssetsFolder, "ion_fuel_cell_01_spec.png")),
					UseIonModelsAsBase = true,
					CustomIllumStrength = 1.5f
				},
				EnhanceGameObject = new Action<GameObject>((go) => {
					var regenerator = go.EnsureComponent<RegeneratingPowerSource>();
					regenerator.SetRegenParameters(IonFuelCellRechargeInterval, IonFuelCellLargeRechargeRate);
				})
			};
			ionFuelCellLarge.Patch();
			RegeneratingPowerSource.StaticSetRegenParameters(IonFuelCellRechargeInterval, IonFuelCellLargeRechargeRate, ionFuelCellLarge.TechType);

			foreach (var item in new[] {
				nBattery.TechType,
				cbLithiumBattery.TechType,
				ionFuelCellSmall.TechType,
			})
				VerifyBattery(item);

			foreach (var item in new[] {
				nPowercell.TechType,
				cbLithiumCell.TechType,
				ionFuelCellLarge.TechType
			})
				VerifyPowerCell(item);
		}

		public static void EnhanceBioCell(GameObject obj)
		{
			obj.EnsureComponent<RegeneratingPowerSource>().SetRegenParameters(0.5f, 1f);
		}
	}
}
