...
namespace TrueSolarPowerCells
{
	internal class SolarRegeneratePowerSource : MonoBehaviour
	{
		...
		protected float baseRegen => TSPCPlugin.config.baseRegenRate;
		...
		private IEnumerator SetupPowerSource()
		{
			...
			yield return new WaitUntil(() => gameObject.TryGetComponent<RegeneratePowerSource>(out component));
			component.regenerationThreshhold = this.regenerationThreshhold;
			component.regenerationAmount = 0f;

			yield return new WaitUntil(() => gameObject.TryGetComponent<PowerSource>(out powerSource));
		...
		private void Update()
		{
			...
			float rechargeAmount = baseRegen + this.regenerationRate * delta * localLightScalar * depthScalar;
